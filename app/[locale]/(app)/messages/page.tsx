...
export default function MessagesPage() {
  ...
  const { messages, loading } = useBrandData(selectedBrand?.id ?? null);
        ...
        <table className="w-full min-w-[860px] text-left text-sm">
          <thead className="text-xs uppercase tracking-wide text-muted">
            <tr>
              ...
              <th className="px-5 py-3 font-medium">{t("body")}</th>
          ...
          <tbody>
            {messages.map((message) => (
              <tr key={message.id} className="border-t border-purple-jam/8">
                ...
                <td className="max-w-sm truncate px-5 py-3 text-muted">{message.body}</td>
